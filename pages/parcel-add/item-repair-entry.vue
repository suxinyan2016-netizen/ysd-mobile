    ...
    <scroll-view class="step-content" scroll-y>
      <view class="info-card">
        ...
        <view class="info-row">
          ...
          <text class="value">{{ item.repairDemands || '-' }}</text>
        </view>
        <view class="info-row">
          <text class="label">维修步骤:</text>
          <text class="value">{{ item.repairProcedure || '-' }}</text>
        </view>
        ...
        <view class="info-row">
          ...
          <textarea class="form-input" v-model="repairResult" placeholder="填写维修结果"></textarea>
        </view>
        <view class="info-row">
          <text class="label">维修费用:</text>
          <input class="form-input" type="text" v-model="repairFeeDisplay" placeholder="0.00" @input="onRepairFeeInput" @blur="formatRepairFee" />
        </view>
      </view>
...
const repairFee = ref(null)
const repairFeeDisplay = ref('')
...
async function load(){
  ...
  try{
    ...
    if (res && res.code === 1 && res.data){
      ...
      item.value = data || {}
      repairResult.value = item.value.repairResult || ''
      repairFee.value = (typeof item.value.repairFee !== 'undefined' && item.value.repairFee !== null) ? Number(item.value.repairFee) : null
      repairFeeDisplay.value = (typeof repairFee.value === 'number' && !isNaN(repairFee.value)) ? repairFee.value.toFixed(2) : '0.00'
      try{
      ...
      }catch(e){ itemImages.value = (item.value.attachments || []).map(a => ({ id: a.id, imageUrl: a.imageUrl || a.fileUrl, thumbnailUrl: a.thumbnailUrl || a.imageUrl })) }
...
function onRepairFeeInput(e){
  try{
    ...
    repairFeeDisplay.value = str
    // sanitize for numeric parsing (allow digits, dot and minus)
    const cleaned = str.replace(/,/g, '').replace(/[^0-9.\-]/g, '').trim()
    const parsed = cleaned === '' ? NaN : parseFloat(cleaned)
    repairFee.value = (!isNaN(parsed) ? parsed : null)
  }catch(err){ console.warn('onRepairFeeInput parse failed', err) }
}

function formatRepairFee(){
  try{
    if (typeof repairFee.value === 'number' && !isNaN(repairFee.value)) repairFeeDisplay.value = repairFee.value.toFixed(2)
    else repairFeeDisplay.value = ''
  }catch(err){ console.warn('formatRepairFee failed', err) }
}

async function uploadImage(filePath, recordId, tempKey){ try{ return await uploadFile(filePath, 'ITEM', (typeof recordId !== 'undefined' && recordId !== null) ? recordId : -1, 'ITEM_REPAIR', tempKey) }catch(e){ throw e } }
...
async function handleSave(){ if (isSaving.value) return null; isSaving.value = true; uni.showLoading({ title: '保存中...' })
  try{
    ...
    const payload = { isGood: (isGoodIndex.value===0?1:0), repairResult: repairResult.value || null, repairFee: (typeof repairFee.value !== 'undefined' && repairFee.value !== null) ? Number(repairFee.value) : null, tempKey: item.value.tempKey }
    let existingId = item.value.itemId || itemId || null
    let res
    if (existingId || fromServiceFlag) {
      if (!existingId && item.value.tempKey) {
        try{ const q = await ApiHelper.get('/items', { tempKey: item.value.tempKey, receiveParcelId: item.value.receiveParcelId, createBy: userStore.userInfo?.name || userStore.userInfo?.id, pageSize: 1 }); if (q && q.code === 1 && q.data && Array.isArray(q.data.rows) && q.data.rows.length) { const found = q.data.rows[0]; existingId = found.itemId || found.id || null; if (existingId) item.value.itemId = existingId } }catch(e){ console.warn('tempKey lookup failed before PUT', e) }
      }
      if (existingId) { res = await ApiHelper.put('/items', { ...payload, itemId: existingId }); console.debug('[item-repair-entry] save PUT /items response:', res) }
    }
    if (!res) {
      const createPayload = { ...payload, itemNo: item.value.itemNo, sellerPart: item.value.sellerPart, dictId: item.value.dictId || null, qty: item.value.qty || item.value.quantity || 1, receiveParcelId: item.value.receiveParcelId || null, receivePackageNo: item.value.receivePackageNo || null, createBy: userStore.userInfo?.name, keeperId: userStore.userInfo?.id || null, ownerId: item.value.ownerId || null, receivedDate: new Date().toISOString().split('T')[0], itemStatus: typeof item.value.itemStatus !== 'undefined' ? item.value.itemStatus : 0 }
      console.debug('[item-repair-entry] create payload:', createPayload)
      res = await ApiHelper.post('/items', createPayload)
      console.debug('[item-repair-entry] save POST /items response:', res)
    }
    if (!(res && res.code === 1)) { const serverMsg = res && (res.msg || res.message || JSON.stringify(res)); uni.hideLoading(); uni.showToast({ title: '保存失败: ' + (serverMsg || ''), icon: 'none' }); return null }
    ...
    if (!recordId) {
      ...
      if (!recordId && item.value.tempKey) {
        try{ const q = await ApiHelper.get('/items', { tempKey: item.value.tempKey, receiveParcelId: item.value.receiveParcelId, createBy: userStore.userInfo?.name || userStore.userInfo?.id, pageSize: 1 }); if (q && q.code === 1 && q.data && Array.isArray(q.data.rows) && q.data.rows.length) { const found = q.data.rows[0]; recordId = found.itemId || found.id || null } }catch(qe){ console.warn('tempKey lookup failed', qe) }
      }
...
async function handleSubmit(){ if (isSaving.value) return; isSaving.value = true; uni.showLoading({ title: '提交中...' })
  try{
    ...
    const updatePayload = { itemId: id, isRepaired: 1, isGood: (isGoodIndex.value===0?1:0), repairResult: repairResult.value || null, repairFee: (typeof repairFee.value !== 'undefined' && repairFee.value !== null) ? Number(repairFee.value) : null }
    const res = await ApiHelper.put('/items', updatePayload)
    console.debug('[item-repair-entry] submit PUT /items response:', res)
    if (!(res && res.code === 1)) { const srv = res && (res.msg || res.message || JSON.stringify(res)); uni.hideLoading(); uni.showToast({ title: '提交失败: ' + (srv || ''), icon: 'none' }); return }
